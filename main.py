...
@register(
    ...
    "2.8.4",
    "https://github.com/shskjw/astrbot_plugin_sbhtest",
)
class FigurineProPlugin(Star):
    ...
    def _mask_api_key(self, key: str) -> str:
        ...
        if not key:
            return "<empty>"
        visible_len = 6 if key.startswith("sk-") else min(6, max(2, len(key) // 4))
        visible = key[:visible_len]
        masked_len = max(8, len(key) - visible_len)
        return f"{visible}{'*' * masked_len}"

    def _normalize_api_profile(self, profile: Any) -> Optional[Dict[str, Any]]:
        if not isinstance(profile, dict):
            return None

        name = str(profile.get("name") or profile.get("id") or "").strip()
        mode = str(profile.get("mode") or profile.get("api_mode") or "generic").strip()
        if mode not in {"generic", "gemini_official"}:
            return None

        api_url = str(profile.get("api_url") or profile.get("url") or profile.get("base_url") or "").strip()
        keys = profile.get("api_keys", profile.get("keys", profile.get("api_key", [])))
        ...
        text_to_image_api_url = str(profile.get("text_to_image_api_url") or "").strip()
        text_to_image_api_keys = profile.get("text_to_image_api_keys", [])
        if isinstance(text_to_image_api_keys, str):
            text_to_image_api_keys = [
                k.strip() for k in re.split(r"[,，\s]+", text_to_image_api_keys) if k.strip()
            ]
        elif isinstance(text_to_image_api_keys, list):
            text_to_image_api_keys = [str(k).strip() for k in text_to_image_api_keys if str(k).strip()]
        else:
            text_to_image_api_keys = []

        if not name or not api_url or not keys:
            return None
        ...
        return {
            ...
            "api_url": api_url,
            "api_keys": keys,
            "text_to_image_api_url": text_to_image_api_url,
            "text_to_image_api_keys": text_to_image_api_keys,
        }

    def _get_api_profiles(self) -> List[Dict[str, Any]]:
        profiles = []
        for item in self.conf.get("api_profiles", []) or []:
            normalized = self._normalize_api_profile(item)
            if normalized:
                profiles.append(normalized)
        return profiles

    def _find_api_profile(self, target: str) -> Optional[Dict[str, Any]]:
        ...
        for profile in profiles:
            if profile["name"].lower() == target_lower:
                return profile
        return None
    ...
    def _apply_api_profile(self, profile: Dict[str, Any]) -> None:
        ...
        self.conf[f"{base}_api_url"] = profile["api_url"]
        self.conf[f"{base}_api_keys"] = list(profile["api_keys"])

        if profile.get("text_to_image_api_url") or profile.get("text_to_image_api_keys"):
            self.conf["text_to_image_api_url"] = profile.get("text_to_image_api_url", "")
            self.conf["text_to_image_api_keys"] = list(profile.get("text_to_image_api_keys", []))
        else:
            self.conf["text_to_image_api_url"] = ""
            self.conf["text_to_image_api_keys"] = []

        self.conf["current_api_profile"] = profile["name"]

    def _format_api_profile_summary(self, profile: Dict[str, Any], index: int = 0, current_name: str = "") -> str:
        mark = " ✅" if current_name and profile["name"] == current_name else ""
        prefix = f"{index}. " if index else ""
        keys = profile.get("api_keys", [])
        t2i_keys = profile.get("text_to_image_api_keys", [])
        t2i = ""
        if profile.get("text_to_image_api_url") or t2i_keys:
            t2i = f"\n   文生图专用: {profile.get('text_to_image_api_url') or '同主站'} | Key {len(t2i_keys)}"
        return (
            f"{prefix}{profile['name']}{mark}\n"
            f"   模式: {profile['mode']}\n"
            f"   当前模型: {self.conf.get('model', 'nano-banana')}\n"
            f"   地址: {profile['api_url']}\n"
            f"   Key: {len(keys)} 个 ({self._mask_api_key(keys[0]) if keys else '无'})"
            f"{t2i}"
        )
    ...
    def _llm_start_result(self, text: str, progress_sent: bool, allow_followup: bool = False) -> str:
        """返回给 LLM 的任务启动状态；已主动发进度提示时阻止二次等待回复。"""
        if progress_sent and not allow_followup:
            return (
                f"{text}\n"
                "【对话要求】插件已经主动给用户发送过等待提示了。"
                "这次工具调用结束后不要再回复任何等待、收到、马上好之类的文字，保持静默即可；"
                "后续图片、文件或汇总会由插件主动发送。"
            )
        return self._finalize_llm_tool_result(text)
    ...
    async def text_to_image_tool(self, event: AstrMessageEvent, prompt: str, count: int = 1):
        ...
        if rebellious_hint:
            ...
            if show_llm_progress:
                return self._llm_start_result(_draw_hint, progress_sent=True)
            return _draw_hint
        else:
            if count_limit_reply:
                return self._llm_start_result(
                    f"[TOOL_SUCCESS] 正在画图中。{count_limit_reply} 请用自然语气回复一句等待的话。",
                    progress_sent=show_llm_progress,
                )
            if count > 1:
                return self._llm_start_result(
    ...
    async def image_edit_tool(self, event: AstrMessageEvent, prompt: str, use_message_images: bool = True,
        ...
        if rebellious_hint:
            ...
            if show_llm_progress:
                return self._llm_start_result(_edit_hint, progress_sent=True)
            return _edit_hint
        else:
            if count_limit_reply:
                return self._llm_start_result(
    ...
    @filter.command("重发最后一张图", aliases={"重发图片", "重发最后图片", "再发一次", "重发上一张图"}, prefix_optional=True)
    async def on_resend_last_generated_image(self, event: AstrMessageEvent, ctx=None):
        """重发当前会话最近一次成功生成的图片。"""
        images = await self._get_recent_generated_images(event.unified_msg_origin, max_images=1)
        if not images:
            yield event.chain_result([Plain("当前会话还没有可重发的生成图。")])
            return

        try:
            yield event.chain_result([Image.fromBytes(images[-1]), Plain(" ")])
        except Exception as e:
            logger.error(f"重发最后生成图失败: {e}", exc_info=True)
            yield event.chain_result([Plain(f"重发失败了: {self._resolve_debug_error_message(e, '刚才发送失败了，稍后再试试。')}")])
    ...
    async def on_switch_mode(self, event: AstrMessageEvent, ctx=None):
        ...
        if mode in ["generic", "gemini_official"]:
            self.conf["api_mode"] = mode;
            self.conf["current_api_profile"] = ""
            self._save_config()
            yield event.chain_result([Plain(f"✅ 已切换为 {mode}")])
        else:
            yield event.chain_result([Plain("模式无效 (generic / gemini_official)")])

    @filter.command("API组合列表", aliases={"api组合列表", "手办化API组合列表", "手办化API列表"}, prefix_optional=True)
    async def on_api_profile_list(self, event: AstrMessageEvent, ctx=None):
        if not self.is_admin(event):
            return

        profiles = self._get_api_profiles()
        current_name = str(self.conf.get("current_api_profile", "") or "")
        if not profiles:
            yield event.chain_result([Plain(
                "暂无 API 组合。\n"
                "用法: #API组合添加 <名称> <generic|gemini_official> <API地址> <Key1> [Key2...]"
            )])
            return

        msg = "📡 API 组合列表:\n" + "\n\n".join(
            self._format_api_profile_summary(profile, i + 1, current_name)
            for i, profile in enumerate(profiles)
        )
        msg += "\n\n💡 切换: #API组合切换 <名称或序号>"
        yield event.chain_result([Plain(msg)])

    @filter.command("API组合当前", aliases={"api组合当前", "当前API组合", "手办化API当前"}, prefix_optional=True)
    async def on_api_profile_current(self, event: AstrMessageEvent, ctx=None):
        if not self.is_admin(event):
            return

        current_name = str(self.conf.get("current_api_profile", "") or "").strip()
        profile = self._find_api_profile(current_name) if current_name else None
        if profile:
            yield event.chain_result([Plain("当前 API 组合:\n" + self._format_api_profile_summary(profile))])
            return

        mode = self.conf.get("api_mode", "generic")
        base = "gemini" if mode == "gemini_official" else "generic"
        keys = self.conf.get(f"{base}_api_keys", []) or []
        msg = (
            "当前 API 配置未绑定到已保存组合。\n"
            f"模式: {mode}\n"
            f"地址: {self.conf.get(f'{base}_api_url', '')}\n"
            f"Key: {len(keys)} 个 ({self._mask_api_key(keys[0]) if keys else '无'})"
        )
        yield event.chain_result([Plain(msg)])

    @filter.command("API组合添加", aliases={"api组合添加", "手办化API组合添加", "手办化API添加"}, prefix_optional=True)
    async def on_api_profile_add(self, event: AstrMessageEvent, ctx=None):
        if not self.is_admin(event):
            return
        ...
        if len(parts) < 5:
            yield event.chain_result([Plain(
                "用法: #API组合添加 <名称> <generic|gemini_official> <API地址> <Key1> [Key2...]\n"
                "示例: #API组合添加 blt generic https://api.example.com sk-xxx"
            )])
            return

        name, mode, api_url = parts[1].strip(), parts[2].strip(), parts[3].strip()
        keys = [p.strip() for p in parts[4:] if p.strip()]
        profile = self._normalize_api_profile({
            "name": name,
            "mode": mode,
            "api_url": api_url,
            "api_keys": keys,
        ...
        if not profile:
            yield event.chain_result([Plain("组合格式不对：模式只能是 generic / gemini_official，且名称、地址、Key 都不能为空。")])
            return
        ...
        self.conf["api_profiles"] = profiles
        self._save_config()
        action = "更新" if replaced else "添加"
        yield event.chain_result([Plain(f"✅ 已{action} API 组合: {profile['name']} ({profile['mode']}, {len(profile['api_keys'])} 个 Key)")])

    @filter.command("API组合切换", aliases={"api组合切换", "切换API组合", "手办化API切换"}, prefix_optional=True)
    async def on_api_profile_switch(self, event: AstrMessageEvent, ctx=None):
        if not self.is_admin(event):
            return

        parts = event.message_str.split(maxsplit=1)
        if len(parts) < 2:
            yield event.chain_result([Plain("用法: #API组合切换 <名称或序号>\n先用 #API组合列表 查看可用组合。")])
            return

        profile = self._find_api_profile(parts[1])
        if not profile:
            yield event.chain_result([Plain("没找到这个 API 组合，先用 #API组合列表 看一下名称或序号。")])
            return

        self._apply_api_profile(profile)
        self._save_config()
        yield event.chain_result([Plain(
            f"✅ 已切换 API 组合: {profile['name']}\n"
            f"模式: {profile['mode']}\n"
            f"地址: {profile['api_url']}\n"
            f"Key: {len(profile['api_keys'])} 个"
        )])

    @filter.command("API组合删除", aliases={"api组合删除", "删除API组合", "手办化API删除"}, prefix_optional=True)
    async def on_api_profile_delete(self, event: AstrMessageEvent, ctx=None):
        if not self.is_admin(event):
            return

        parts = event.message_str.split(maxsplit=1)
        if len(parts) < 2:
            yield event.chain_result([Plain("用法: #API组合删除 <名称或序号>")])
            return

        target = parts[1].strip()
        profiles = self._get_api_profiles()
        profile = self._find_api_profile(target)
        if not profile:
            yield event.chain_result([Plain("没找到这个 API 组合。")])
            return

        profiles = [item for item in profiles if item["name"].lower() != profile["name"].lower()]
        self.conf["api_profiles"] = profiles
        if self.conf.get("current_api_profile") == profile["name"]:
            self.conf["current_api_profile"] = ""
        self._save_config()
        yield event.chain_result([Plain(f"✅ 已删除 API 组合: {profile['name']}")])
        ...
        if count == 1:
            ...
            if count_limited:
                return self._llm_start_result(
                    ...
                    progress_sent=self._get_conf_bool("llm_show_progress", True),
                )
            return self._llm_start_result(
