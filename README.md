...
如果同时安装并启用了 `astrbot_plugin_life_scheduler`，人设拍照会尝试读取它当天生成的 `outfit` 穿搭信息，并临时追加到 shoubanhua 的人设描述中，让人设照片里的衣着尽量与每日日程保持一致。

说明：

- 联动默认开启，配置项为 `persona_use_life_scheduler_outfit`。
- 日程插件不存在、未启用、今日还没生成日程，或读取不到 `outfit` 时，会自动跳过，不影响原本的人设拍照。
- 这个联动不会永久改写 `persona_description`，只在构建本次人设拍照提示词时临时追加今日穿搭。
- 管理员可用 `#人设日程穿搭` 查看当前读取到的穿搭内容。
